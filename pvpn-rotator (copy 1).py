...
def connect_to_server(server_id):
    ...
    try:
        ...
        return False, f"Failed to connect to {server_id}: {e.stderr.decode()}"
...
def get_current_connection():
    """Returns currently connected server or None."""
    result = run(["protonvpn", "status"], capture_output=True, text=True)
...
class VPNRotatorDaemon:
    ...
    def log_status(self):
        ...
        current_server = get_current_connection()
        ...
        while not self.shutdown:
            try:
                ...
                if not self.paused:
                    ...
                    # Connect
                    success, msg = connect_to_server(server)
                    now = datetime.now()
                    with open(LOG_FILE, 'a') as log:
                    ...
                    if int(time.time()) % 30 == 0:  # Log every 30 seconds when paused
                        with open(LOG_FILE, 'a') as log:
                            log.write(f"{datetime.now()}: Daemon paused. Current VPN: {get_current_connection()}\n")
                    time.sleep(1)
